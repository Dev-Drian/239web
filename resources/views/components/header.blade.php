<div class="glass-dark border border-white/15 rounded-xl shadow-lg">
    ...
    <div class="hidden sm:flex items-center justify-between py-3 px-6 max-w-7xl mx-auto">
        ...
        <div class="flex items-center space-x-3">
            <img src="{{ asset('img/logo.png') }}" alt="Logo" class="h-8 md:h-9 rounded-md transition-transform duration-300 hover:scale-105">
            <div class="text-slate-400 font-light">|</div>
        ...
        <div class="flex items-center space-x-4 md:space-x-5">
            ...
            <div x-data="{ open: false }" class="relative">
                <button @click="open = !open" type="button"
                    ...
                    <img class="h-9 w-9 rounded-full object-cover shadow-sm group-hover:shadow-md transition-all duration-300 ring-2 ring-white/20"
                        src="{{ auth()->user()->profile_photo_url ?? asset('img/default-avatar.png') }}" alt="User profile">
                    <div class="flex flex-col text-left ml-1">
                        <span class="font-medium text-white group-hover:text-slate-200 hidden md:inline transition-colors duration-300">{{ auth()->user()->name }}</span>
                        <span class="text-xs text-slate-400 hidden md:inline">{{ auth()->user()->email }}</span>
                    </div>
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-slate-400 group-hover:text-slate-300 transition-colors duration-300" viewBox="0 0 20 20"
                        fill="currentColor">
                ...
                <div x-show="open" @click.away="open = false" x-transition:enter="transition ease-out duration-200"
                    ...
                    class="absolute right-0 mt-3 w-60 origin-top-right rounded-xl glass-dark py-2 shadow-xl ring-1 ring-white/15 focus:outline-none z-50 border border-white/10"
                    role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabindex="-1">
                    <div class="px-4 py-2 md:hidden border-b border-white/15">
                        <p class="text-sm font-medium text-white">{{ auth()->user()->name }}</p>
                        <p class="text-xs text-slate-400 truncate">{{ auth()->user()->email }}</p>
                    </div>
                    <a href="{{ route('profile.show') }}" class="flex items-center px-4 py-3 text-sm text-slate-300 hover:bg-white/10 hover:text-white transition-colors duration-200 rounded-lg mx-2"
                        role="menuitem">
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit"
                            class="flex items-center w-full text-left px-4 py-3 text-sm text-slate-300 hover:bg-white/10 hover:text-white transition-colors duration-200 rounded-lg mx-2"
                            role="menuitem">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-3 text-slate-400" viewBox="0 0 20 20" fill="currentColor">
    ...
    <div class="sm:hidden">
        ...
        <div class="flex items-center justify-between py-3 px-4">
            ...
            <div class="flex items-center flex-shrink-0">
                ...
                <div class="mx-2 text-slate-400 font-light">|</div>
                <h2 class="text-base font-semibold text-white truncate max-w-[150px]">
                    {{ __($name) }}
                </h2>
            </div>
            ...
            <div class="flex items-center space-x-4">
                ...
                <div x-data="{ open: false }" class="relative">
                    <button @click="open = !open" type="button"
                        ...
                        <img class="h-8 w-8 rounded-full object-cover shadow-sm hover:shadow-md transition-all duration-300 ring-2 ring-white/20"
                            src="{{ auth()->user()->profile_photo_url ?? asset('img/default-avatar.png') }}" alt="User profile">
                    </button>
