<script>
    function setupImageGeneration() {
        ...
        // Flags globales para prevenir múltiples peticiones
        ...
        function cleanupPreviousListeners() {
            ...
            // Remover todos los event listeners del botón de generar
            ...
            // Remover todos los event listeners del botón de upload
        ...
        function switchTab(tab) {
            if (tab === 'ai') {
                elements.aiTab?.classList.add('active', 'text-blue-600', 'border-blue-500');
                elements.aiTab?.classList.remove('text-gray-500', 'hover:text-gray-700', 'hover:border-gray-300',
                    'border-transparent');
                elements.aiTabContent?.classList.remove('hidden');

                elements.uploadTab?.classList.remove('active', 'text-blue-600', 'border-blue-500');
                elements.uploadTab?.classList.add('text-gray-500', 'hover:text-gray-700', 'hover:border-gray-300',
                    'border-transparent');
                elements.uploadTabContent?.classList.add('hidden');
            } else {
                elements.uploadTab?.classList.add('active', 'text-blue-600', 'border-blue-500');
                elements.uploadTab?.classList.remove('text-gray-500', 'hover:text-gray-700', 'hover:border-gray-300',
                    'border-transparent');
                elements.uploadTabContent?.classList.remove('hidden');

                elements.aiTab?.classList.remove('active', 'text-blue-600', 'border-blue-500');
                elements.aiTab?.classList.add('text-gray-500', 'hover:text-gray-700', 'hover:border-gray-300',
                    'border-transparent');
        ...
        function initializeDragAndDrop() {
            ...
            function highlight() {
                elements.dropZone?.classList.add('border-blue-500', 'bg-blue-100');
                elements.dropZone?.classList.remove('border-gray-300', 'bg-gray-50');
            }

            function unhighlight() {
                elements.dropZone?.classList.remove('border-blue-500', 'bg-blue-100');
            ...
            reader.onload = function(e) {
                ...
                if (elements.generatedImageContainer) {
                    elements.generatedImageContainer.innerHTML =
                        `<img src="${e.target.result}" alt="Preview" class="w-full h-auto rounded-lg shadow-lg">`;
                }
            };
        ...
        // AI Image Generation - COMPLETAMENTE REESCRITO
        function initializeImageGeneration() {
            ...
            elements.generateImageBtn.addEventListener('click', handleGenerateClick, {
                once: false
            });
        ...
        function handleGenerateClick(event) {
            ...
            // Marcar inmediatamente como generando
        ...
        function generateImageRequest(prompt, formData) {
            ...
            console.log('📤 Datos a enviar:', {
                ...
                output_format: formData.get('output_format') || 'jpeg',
                aspect_ratio: formData.get('image_ratio') || '16:9'
            });
        ...
        function handleImageGenerationResponse(data) {
            ...
            // Validar URL
            if (!imageUrl || (!imageUrl.startsWith('http://') && !imageUrl.startsWith('https://') && !imageUrl
                    .startsWith('data:'))) {
        ...
        function initializeImageUpload() {
            ...
            elements.uploadImageBtn.addEventListener('click', function(e) {
                ...
                if (window.featuredImageUrl) {
                    showAlert('warning',
                        'An image has already been processed. Please reset the image first to upload a new one.'
                    );
                    return;
                }
                ...
                .then(data => {
                    ...
                    setTimeout(() => {
                        ...
                        if (data.success && data.data && data.data.image_url) {
                            ...
                            showAlert('error', 'Error processing image: ' + (data.message ||
        ...
        function handleImageSuccess(imageUrl, type) {
            ...
            // SOLUCIÓN SIMPLE - SIN OVERLAYS NI EFECTOS
            if (elements.generatedImageContainer) {
                elements.generatedImageContainer.innerHTML =
                    `<img src="${imageUrl}" 
                  alt="Generated Image" 
        ...
        function resetImageContainer() {
            if (elements.generatedImageContainer) {
                ...
                <div class="flex flex-col items-center justify-center py-12 text-gray-400">
                    ...
                    <p class="text-center font-medium">No image generated yet</p>
                    <p class="text-sm text-gray-400 mt-1">Generate with AI or upload your own</p>
                </div>
            `;
            }
        }
