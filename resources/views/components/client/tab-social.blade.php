<div id="social-tab" class="tab-content hidden">
    <div class="p-8 glass-dark rounded-2xl" id="tab-social">
        <div class="mb-8">
            <h2 class="text-2xl font-bold text-white mb-2">Social Media Links</h2>
            <p class="text-slate-300">Connect your social media profiles to boost your online presence</p>
        </div>
        
        <!-- Add New Social Link Form -->
        <div class="glass p-6 rounded-xl shadow-lg border border-white/20 mb-8 hover:shadow-xl transition-all duration-300">
            <div class="flex flex-col lg:flex-row gap-4 items-end">
                <div class="flex-1 min-w-0">
                    <label class="block text-sm font-semibold text-slate-300 mb-3">Choose Platform</label>
                    <div class="relative">
                        <select id="social_type" class="w-full glass border-2 border-white/20 rounded-xl px-4 py-3 pr-12 bg-transparent text-white focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-200 appearance-none cursor-pointer hover:border-white/30">
                            <option value="" class="text-black bg-white">Select a platform...</option>
                            <option value="facebook" class="text-black bg-white">Facebook</option>
                            <option value="instagram" class="text-black bg-white">Instagram</option>
                            <option value="linkedin" class="text-black bg-white">LinkedIn</option>
                            <option value="twitter" class="text-black bg-white">Twitter / X</option>
                            <option value="youtube" class="text-black bg-white">YouTube</option>
                            <option value="tiktok" class="text-black bg-white">TikTok</option>
                            <option value="whatsapp" class="text-black bg-white">WhatsApp</option>
                            <option value="telegram" class="text-black bg-white">Telegram</option>
                            <option value="discord" class="text-black bg-white">Discord</option>
                            <option value="website" class="text-black bg-white">Website</option>
                        </select>
                ...
                <div class="flex-2 min-w-0">
                    <label class="block text-sm font-semibold text-slate-300 mb-3">Profile URL</label>
                    <div class="relative">
                        <input type="url" id="social_url" 
                               class="w-full glass border-2 border-white/20 rounded-xl px-4 py-3 bg-transparent text-white placeholder-slate-400 focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-200 hover:border-white/30" 
                        ...
                        <div id="url_preview" class="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none opacity-0 transition-opacity duration-200">
                            <div class="w-5 h-5 rounded-full bg-slate-400"></div>
                        </div>
                    </div>
                </div>
        ...
        <div class="space-y-4" id="socialLinksContainer">
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-lg font-semibold text-white">Your Social Links</h3>
            ...
            <div id="emptyState" class="text-center py-16 px-6">
                <div class="w-24 h-24 mx-auto mb-6 bg-gradient-to-br from-indigo-500/20 to-purple-500/20 rounded-full flex items-center justify-center">
                ...
                <h4 class="text-xl font-semibold text-white mb-2">No social links yet</h4>
                <p class="text-slate-300 mb-6 max-w-md mx-auto">Start building your social presence by adding your first social media profile link above.</p>
                <div class="flex flex-wrap justify-center gap-2">
                    <span class="px-3 py-1 bg-indigo-500/20 text-indigo-300 rounded-full text-sm font-medium">Facebook</span>
                    <span class="px-3 py-1 bg-pink-500/20 text-pink-300 rounded-full text-sm font-medium">Instagram</span>
                    <span class="px-3 py-1 bg-blue-500/20 text-blue-300 rounded-full text-sm font-medium">LinkedIn</span>
                    <span class="px-3 py-1 bg-red-500/20 text-red-300 rounded-full text-sm font-medium">YouTube</span>
                </div>
            </div>
        </div>
    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function createSocialCard(type, url) {
            ...
            card.className = 'group glass rounded-xl p-6 shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1';
            card.innerHTML = `
                <div class="flex items-start justify-between mb-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h4 class="font-semibold text-white">${platform.name}</h4>
                            <p class="text-sm text-slate-300">Social Profile</p>
                        </div>
                    </div>
                    <button type="button" class="opacity-0 group-hover:opacity-100 text-slate-400 hover:text-red-400 p-2 rounded-lg hover:bg-red-500/10 transition-all duration-200" 
                ...
                <div class="space-y-3">
                    <a href="${url}" target="_blank" 
                       class="block text-sm text-slate-300 hover:${platform.textColor} transition-colors duration-200 truncate font-medium">
                        ${url}
                    </a>
                    <div class="flex gap-2">
                        ...
                        <button type="button" onclick="copyToClipboard('${url}')" 
                                class="px-4 py-2 glass border border-white/20 rounded-lg text-sm font-medium text-slate-300 hover:bg-white/10 transition-all duration-200">
                            Copy
                        </button>
                    </div>
                </div>
