    ...
    <div :class="{
        ...
        <div class="flex h-16 items-center justify-between px-4 border-b border-white/15">
            <a href="{{ route('dashboard') }}" class="flex items-center space-x-3 group">
                <div
                    class="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-blue-500 to-indigo-600 text-white transition-all duration-300 hover:scale-105 shadow-md">
        ...
        <div class="flex flex-col h-[calc(100vh-8rem)] overflow-y-auto py-4 px-3 beautiful-scrollbar">
            <nav class="flex-grow space-y-1">
                ...
                <a href="{{ route('dashboard') }}"
                    ...
                    <div x-show="!open"
                        class="absolute left-14 bg-slate-800 text-white px-2 py-1 rounded text-xs opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-50">
                        {{ __('Dashboard') }}
                    </div>
                ...
                <a href="{{ route('service.index') }}"
                    class="group flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-all duration-200 relative"
                    :class="activeSection === 'service.index' ?
                        'bg-blue-600/20 text-blue-300 border border-blue-500/30 shadow-lg' :
                        'text-slate-300 hover:bg-white/10 hover:text-blue-300'"
                    @click="activeSection = 'service.index'">
                    <div class="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg transition-all duration-200"
                        :class="open ? 'mr-3' : 'mx-auto'">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor" stroke-width="2">
                    ...
                    <span x-show="open" class="font-medium truncate">{{ __('Services') }}</span>
                    <div x-show="!open"
                        class="absolute left-14 bg-slate-800 text-white px-2 py-1 rounded text-xs opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-50">
                        {{ __('Services') }}
                    </div>
                    <span x-show="open && activeSection === 'service.index'"
                        class="ml-auto h-2 w-2 rounded-full bg-blue-400 animate-pulse"></span>
                </a>
                <!-- Blog -->
                ...
                <a href="{{ route('keyword.index') }}"
                    class="group flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-all duration-200 relative"
                    :class="activeSection === 'keyword.index' ?
                        'bg-blue-600/20 text-blue-300 border border-blue-500/30 shadow-lg' :
                        'text-slate-300 hover:bg-white/10 hover:text-blue-300'"
                    @click="activeSection = 'keyword.index'">
                    <div class="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg transition-all duration-200"
                        :class="open ? 'mr-3' : 'mx-auto'">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor" stroke-width="2">
                    ...
                    <span x-show="open" class="font-medium truncate">{{ __('Keywords') }}</span>
                    <div x-show="!open"
                        class="absolute left-14 bg-slate-800 text-white px-2 py-1 rounded text-xs opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-50">
                        {{ __('Keywords') }}
                    </div>
                    <span x-show="open && activeSection === 'keyword.index'"
                        class="ml-auto h-2 w-2 rounded-full bg-blue-400 animate-pulse"></span>
                </a>
                <!-- Chat -->
                <a href="{{ route('chat.index') }}"
                    ...
                    <div class="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg transition-all duration-200"
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M17 8h2a2 2 0 012 2v6a2 2 0 01-2 2h-2v4l-4-4H9a2 2 0 01-2-2v-1M5 8h2m0 0V6a2 2 0 012-2h6a2 2 0 012 2v2m-8 0h8" />
                        </svg>
                    </div>
                ...
                <div class="relative">
                    <button @click="todoDropdown = !todoDropdown"
                        ...
                        <svg x-show="open" class="ml-2 h-4 w-4 transition-transform duration-200"
                            :class="{ 'rotate-180': todoDropdown }" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor" stroke-width="2">
                ...
                <div class="relative">
                    <button @click="generateDropdown = !generateDropdown"
                        ...
                        <span x-show="open"
                            class="font-medium truncate flex-1 text-left">{{ __('Generate') }}</span>
                        <svg x-show="open" class="ml-2 h-4 w-4 transition-transform duration-200"
                            :class="{ 'rotate-180': generateDropdown }" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
                        </svg>
                        <div x-show="!open"
                    ...
                    <div x-show="generateDropdown && open" x-transition:enter="transition ease-out duration-200"
                        ...
                        <a href="{{ route('imagesUser.index') }}"
                            ...
                            <svg class="mr-2 h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                                stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="M4 16l4.707-4.707a1 1 0 011.414 0L16 17m-2-5a1 1 0 011.414 0L20 17M6 2h8a2 2 0 012 2v6a2 2 0 01-2 2H6a2 2 0 01-2-2V4a2 2 0 012-2z" />
                            </svg>
                            {{ __('Images') }}
                        </a>
                        <a href="{{ route('generate.index') }}"
                            ...
                            <svg class="mr-2 h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                                stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="M7 4V2a1 1 0 011-1h8a1 1 0 011 1v2m-9 4v10a2 2 0 002 2h6a2 2 0 002-2V8M7 8h10M9 12h6" />
                            </svg>
                            {{ __('Prompts') }}
                        </a>
                    </div>
        ...
        <div class="absolute bottom-0 left-0 right-0 border-t border-white/15 glass-dark p-4">
            <div class="flex items-center">
                ...
                @else
                    <div
                        class="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-blue-500 to-indigo-600 text-white">
